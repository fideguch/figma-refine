...
designs/ がない場合、ユーザーに質問して Design Personality を手動構築する。

### 質問フロー

```
⚠ designs/ フォルダが見つかりません。
  プロジェクトに合わせた修正のため、いくつか質問させてください。
  （スキップも可能です — その場合は汎用モードで処理します）
```

| # | 質問 | Design Personality への反映 | スキップ時のデフォルト |
|---|------|---------------------------|---------------------|
| Q1 | ブランドカラー（Primary / Secondary / Accent）を教えてください | brandColors | Tailwind Slate パレット |
| Q2 | フォントは何を使っていますか？（例: Noto Sans JP, Inter） | typography.font | Noto Sans JP |
| Q3 | デザインの雰囲気は？（例: 信頼感、モダン、遊び心、プレミアム） | mood | "クリーン・プロフェッショナル" |
| Q4 | ターゲットユーザーは誰ですか？（年齢層、職種等） | targetUser | "一般ビジネスユーザー" |
| Q5 | 参考にしているサービスやサイトはありますか？ | competitiveRefs | [] |
| Q6 | 特別な制約はありますか？（アクセシビリティ、多言語等） | constraints | ["WCAG 2.1 AA"] |

### 質問の進め方

```
1. Q1〜Q6 を順に提示（まとめて聞いてもよい）
2. ユーザーの回答を Design Personality に反映
3. 「スキップ」「分からない」→ デフォルト値を使用
4. 「全部スキップ」→ 汎用モードで続行
```
